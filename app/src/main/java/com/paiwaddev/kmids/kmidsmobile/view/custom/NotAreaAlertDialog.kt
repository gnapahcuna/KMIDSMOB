...
class NotAreaAlertDialog(private var _activity: Activity) : DialogFragment() {

    companion object {

        const val TAG = "NotAreaDialog"

        private const val KEY_TITLE = "KEY_TITLE"
        private const val KEY_BUTTON = "KEY_BUTTON"

        fun newInstance(title: String, button: String, activity: Activity): NotAreaAlertDialog {
            val args = Bundle()
            args.putString(KEY_TITLE, title)
            args.putString(KEY_BUTTON, button)
            val fragment = NotAreaAlertDialog(activity)
